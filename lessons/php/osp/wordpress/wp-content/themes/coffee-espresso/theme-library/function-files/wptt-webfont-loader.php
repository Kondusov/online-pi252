...
if ( ! class_exists( 'Coffee_Espresso_WPTT_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class Coffee_Espresso_WPTT_WebFont_Loader {
		 ...
		 * @access public
		 * @since 1.1.0
		 * @param string $coffee_espresso_url The remote URL.
		 */
		public function __construct( $coffee_espresso_url = '' ) {
			$this->remote_url = $coffee_espresso_url;
		...
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $font_family => $files ) {
				...
				foreach ( $files as $coffee_espresso_url ) {

					// Get the filename.
					$filename  = basename( wp_parse_url( $coffee_espresso_url, PHP_URL_PATH ) );
					...
					$tmp_path = download_url( $coffee_espresso_url );
			...
			if ( $change ) {
				...
				foreach ( $stored as $coffee_espresso_url => $path ) {
					if ( ! file_exists( $path ) ) {
						unset( $stored[ $coffee_espresso_url ] );
					}
				}
		 ...
		 * @access public
		 * @since 1.0.0
		 * @param string $coffee_espresso_format The format to be used. Use "woff" or "woff2".
		 * @return void
		 */
		public function set_font_format( $coffee_espresso_format = 'woff2' ) {
			$this->font_format = $coffee_espresso_format;
		}
		 ...
		 * @access protected
		 *
		 * @param string $coffee_espresso_url The URL.
		 *
		 * @return string
		 */
		protected function get_absolute_path( $coffee_espresso_url ) {

			// If dealing with a root-relative URL.
			if ( 0 === stripos( $coffee_espresso_url, '/' ) ) {
				$coffee_espresso_parsed_url = parse_url( $this->remote_url );
				return $coffee_espresso_parsed_url['scheme'] . '://' . $coffee_espresso_parsed_url['hostname'] . $coffee_espresso_url;
			}

			return $coffee_espresso_url;
		}
	}
}

if ( ! function_exists( 'coffee_espresso_wptt_get_webfont_styles' ) ) {
	/**
	 ...
	 * @since 1.0.0
	 *
	 * @param string $coffee_espresso_url    The URL of the remote webfont.
	 * @param string $coffee_espresso_format The font-format. If you need to support IE, change this to "woff".
	 *
	 * @return string Returns the CSS.
	 */
	function coffee_espresso_wptt_get_webfont_styles( $coffee_espresso_url, $coffee_espresso_format = 'woff2' ) {
		$coffee_espresso_font = new Coffee_Espresso_WPTT_WebFont_Loader( $coffee_espresso_url );
		$coffee_espresso_font->set_font_format( $coffee_espresso_format );
		return $coffee_espresso_font->get_styles();
	}
}

if ( ! function_exists( 'coffee_espresso_wptt_get_webfont_url' ) ) {
	/**
	 ...
	 * @return string Returns the CSS.
	 */
	function coffee_espresso_wptt_get_webfont_url( $coffee_espresso_url, $coffee_espresso_format = 'woff2' ) {
		$coffee_espresso_font = new Coffee_Espresso_WPTT_WebFont_Loader( $coffee_espresso_url );
		$coffee_espresso_font->set_font_format( $coffee_espresso_format );
		return $coffee_espresso_font->get_url();
	}
}